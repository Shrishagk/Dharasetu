...
const CapabilityCenter = lazy(() => import('./DemoWorkspace').then(({ CapabilityCenter: Page }) => ({ default: Page })));
...
function DemoPage(props: { dashboard?: Dashboard; sources: Source[]; changes: any[]; selectedSourceIds: string[]; refresh: () => Promise<void>; notify: (message: string) => void; runUnlocked: boolean; onRunUnlocked: () => void }) {
  ...
  return <><CapabilityCenter sources={props.sources} jobActive={controlRun} lastRun={props.dashboard?.latest_job} onRun={runFromControlPlane} onOpenSources={() => navigateTo('data-sources')} notify={props.notify} /><ModernDemoPage {...props} /></>;
}
