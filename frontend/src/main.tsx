...
function featureBounds(features: any[]) {
  ...
  useEffect(() => {
    ...
    instance.on('load', async () => {
      try {
        ...
        const bounds = featureBounds(data.features) || (source.bbox?.length === 4 ? new maplibregl.LngLatBounds([source.bbox[0], source.bbox[1]], [source.bbox[2], source.bbox[3]]) : undefined);
        if (bounds) instance.fitBounds(bounds, { padding: 35, maxZoom: 16, duration: 0 });
        setState('ready');
